...
def generate_wordcloud(text, theme="light"):
    ...
    wordcloud = WordCloud(width=800, height=400, background_color=color, colormap='Blues').generate(text[:512])
...
def extract_entities(text):
    ner_results = load_ner_pipeline()(text[:512])
    unique_entities = []
    seen = set()
...
def summarize_text(text):
    summary_result = load_summarization_pipeline()(text[:512])
